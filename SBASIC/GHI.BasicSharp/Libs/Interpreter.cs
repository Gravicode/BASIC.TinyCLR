...
namespace BasicSharp
{
    public class Interpreter
    {
        ...
        private Hashtable  vars; // all variables are stored here
        private Hashtable  labels; // already seen labels 
        private Hashtable loops; // for loops 
        ...
        public delegate Value BasicFunction(Interpreter interpreter, ArrayList args);
        //public delegate Value BasicFunction(Interpreter interpreter, List<Value> args);
        private Hashtable funcs; // all maped functions
        ...
        {
            if (!vars.Contains(name)) vars.Add(name, val);
            else vars[name] = val;
        }
        ...
        {
            if (!funcs.Contains(name)) funcs.Add(name, function);
        ...
        void Input()
        {
            ...
            while (true)
            {
                ...
                if (!vars.Contains(lex.Identifier)) vars.Add(lex.Identifier, new Value());
        ...
        void Goto()
        {
            ...
            if (!labels.Contains(name))
            {
                ...
                while (true)
                {
                    ...
                    {
                        if (!labels.Contains(lex.Identifier))
        ...
        {
            string name = lex.Identifier;
            if (!labels.Contains(name)) labels.Add(name, lex.TokenMarker);
        ...
        void For()
        {
            ...
            if (((Value)vars[var]).BinOp(v, Token.More).Real == 1)
            {
                while (true)
                {
        ...
        void Next()
        {
            ...
            Match(Token.Identifier);
            string var = lex.Identifier;
            vars[var] = ((Value)vars[var]).BinOp(new Value(1), Token.Plus);
            var tmp = (Marker)loops[var];
            lex.GoTo(new Marker(tmp.Pointer - 1, tmp.Line, tmp.Column - 1));
            lastToken = Token.NewLine;
        }
        ...
        Value Expr(int min = 0)
        {
            ...
            Hashtable precedens = new Hashtable();
            precedens.Add(Token.Or, 0);
            precedens.Add(Token.And, 0);
            precedens.Add(Token.Equal, 1);
            precedens.Add(Token.NotEqual, 1);
            precedens.Add(Token.Less, 1);
            precedens.Add(Token.More, 1);
            precedens.Add(Token.LessEqual, 1);
            precedens.Add(Token.MoreEqual, 1);
            precedens.Add(Token.Plus, 2);
            precedens.Add(Token.Minus, 2);
            precedens.Add(Token.Asterisk, 3);
            precedens.Add(Token.Slash, 3);
            precedens.Add(Token.Caret, 4);

            Value lhs = Primary();

            while (true)
            {
                if (lastToken < Token.Plus || lastToken > Token.And || (int)precedens[lastToken] < min)
                    break;

                Token op = lastToken;
                int prec = (int)precedens[lastToken]; // Operator Precedence
        ...
        Value Primary()
        {
            ...
            {
                // ident | ident '(' args ')'
                if (vars.Contains(lex.Identifier))
                {
                    prim = (Value)vars[lex.Identifier];
                }
                else if (funcs.Contains(lex.Identifier))
                {
                    string name = lex.Identifier;
                ...
                start:
                    ...
                    prim = ((BasicFunction)funcs[name])(null, args);
                }
                else
                {
